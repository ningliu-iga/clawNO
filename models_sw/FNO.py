...
class FNO2d(nn.Module):
    def __init__(self, num_channels, modes1, modes2, width, initial_step, grid_type):
        ...
        self.p = nn.Linear(initial_step * num_channels + self.grid.grid_dim,
                           self.width)  # input channel is 12: the solution of the previous 10 timesteps + 2 locations (u(t-10, x, y), ..., u(t-1, x, y),  x, y)
    ...
    def forward(self, x):
        ...
        # x = F.pad(x, (self.padding, self.padding, self.padding, self.padding))  # pad the domain if input is non-periodic
