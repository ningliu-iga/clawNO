...
from models_sw.FNO import FNO2d, FNO3d
from models_sw.clawFNO import clawFNO2d, clawFNO3d
from models_sw.GFNO import GFNO2d, GFNO3d
from models_sw.clawGFNO import clawGFNO2d, clawGFNO3d
# from models.GFNO_steerable import GFNO2d_steer
# from models.Unet import Unet_Rot, Unet_Rot_M, Unet_Rot_3D
from models_sw.unet_orig import UNet1d, UNet2d, UNet3d
from models_sw.Ghybrid import Ghybrid2d
from models_sw.radialNO import radialNO2d, radialNO3d
from models_sw.GCNN import GCNN2d, GCNN3d
...
def main(args):
    ...
    if rdb:
        ...
        assert T == 25, "T should be 25 for ns 5pred1"
        T_in = 5
        S = Sx = Sy = 64
        num_channels = 2  # (u, v)
        S_super = 4 * S  # super spatial res
    ...
    root = args.results_path + 'grid_%s_n%d_new/seed%d_lr%e_wd%e' % (
    ...
    if swe:
        ...
        sin_colat = torch.flip(sin_colat, (1,)).to(device)
    ...
    if i_flag_check_div_free_3d:
        if rdb:
            ...
            i_div_free = div_free_3d_check_speedyweather(data[0:1, ..., 0:1], data[0:1, ..., 1:2], data[0:1, ..., 2:3], lon, colat)
    ...
    for ep in range(epochs):
        ...
        with torch.no_grad():
            ...
            for xx, yy in valid_loader:
                ...
                pred = get_eval_pred(model=model, x=xx, strategy=args.strategy,
                                     T=T, times=eval_times).view(len(xx), Sx, Sy, T, num_channels)
    ...
    with torch.no_grad():
        for xx, yy in test_loader:
            ...
            pred = get_eval_pred(model=model, x=xx, strategy=args.strategy,
                                 T=T, times=[]).view(len(xx), Sx, Sy, T, num_channels)
            if rdb:
                pred[..., 0] /= pred[..., 2]
                pred[..., 1] /= pred[..., 2]
                yy[..., 0] /= yy[..., 2]
            ...
            if swe:
                ...
                yy[..., 2] /= sin_colat
                yy[..., 0] /= yy[..., 2]
            test_l2_converted += lploss(pred.reshape(len(pred), -1, num_channels), yy.reshape(len(yy), -1, num_channels)).item()
            # if swe:
        ...
        #         yy[..., 0] /= yy[..., 2]
        #         yy[..., 1] /= yy[..., 2]
        #     elif swe and iflag_convert_sol:
        #         pred[..., 1] /= pred[..., 2]
        #         pred[..., 2] /= sin_colat
        #         pred[..., 0] /= pred[..., 2]
        #         yy[..., 1] /= yy[..., 2]
    ...
    f = open("res_sw3d_%s_%s_n%d.txt" % (args.model_type, grid_type, args.ntrain), "a")
    f.write(f'{args.learning_rate:e}, {args.lmbda:e}, {args.seed:d}, {train_times}, {best_epoch + 1}, '
            f'{train_l2 / ntrain}, {best_valid / nvalid}, {test_l2 / ntest}, {test_l2_converted / ntest}\n')
    f.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hpc_node", type=int, default=0)
    parser.add_argument("--hpc_node2", type=int, default=0)
    ...
    if args.hpc_node == 0 and args.hpc_node2 == 0:
        f = open("res_sw3d_%s_%s_n%d.txt" % (args.model_type, grid_type, args.ntrain), "w")
        f.write(f'learning_rate, wd, seed, mean epoch time, best epoch, '
                f'train_loss, valid_loss, test_loss, test_loss_converted\n')
        f.close()
