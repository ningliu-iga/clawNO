...
class E_GCL_GKN(nn.Module):
    ...
    def coord_conv_mat2d(self, coord, edge_index, coord_diff, edge_feat, OBMmatrix):
        row, col = edge_index
        #trans = coord_diff * self.coord_mlp(edge_feat)
        ...
        # curl layer (matrix method)
        smdx=OBMmatrix['smdx']
        smdy=OBMmatrix['smdy']
        hx = torch.matmul(smdx, agg)
        hy = torch.matmul(smdy, agg)
        agg = torch.cat((hy, -hx), 1)
    ...
    def forward(self, h, edge_index, coord_curr, edge_attr, OBMmatrix, node_attr=None):
        row, col = edge_index
        ...
        coord_curr = self.coord_conv_mat2d(coord_curr, edge_index, coord_diff, edge_feat, OBMmatrix)
